...
#define STOP 0
#define CHAINBAR_MIDPOINT 3700
#define CHAINBAR_HORIZONTAL 1465
#define CHAINBAR_VERTICAL 3700
#define MAX_CONE_DISTANCE 17
#define MAX_ELEVATION 1840
#define MAX_CONES 17
#define DOWN 5
#define OFFSET 4100
...
void chainbarHold(void){
	if(SensorValue(potChainbar) > CHAINBAR_MIDPOINT){
		setChainbar(17);
	}
	else if(SensorValue(potChainbar) < CHAINBAR_MIDPOINT){
		setChainbar(-17);
	}
	else{
		setChainbar(STOP);
...
void setPositionDR(int angle){
	while(SensorValue[potElevation]!= angle){
		if(4100 - SensorValue(potElevation)<angle){
			setElevation(127);
		}
		else if(4100 - SensorValue(potElevation)>angle){
			setElevation(-127);
		}
	}
	elevationHold();
...
bool sonarElevation(void){
	while(SensorValue(coneSonar) < MAX_CONE_DISTANCE){
		...
		//coneCounter++;
		setElevation(127);
		if(4100 - SensorValue(potElevation) > MAX_ELEVATION){//Worst Case
			setElevation(STOP);
			return true;
		}
	}
