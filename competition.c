...
#pragma config(Sensor, dgtl5,  towerSensor,    sensorSONAR_cm)
...
#pragma config(Motor,  port2,           gripperBottom, tmotorVex393_MC29, openLoop)
...
#pragma config(Motor,  port9,           gripperTop,    tmotorVex393_MC29, openLoop)
...
//Valores iniciales del potenciometro (2R4B)
int drMax = 2055;
int drMin = 245;

// controla los rollers
void roll(bool grab)
{
	int speed;
	if(grab) speed = 127;
	else speed = -127;
	motor[gripperTop] = speed;
	wait1Msec(10); // cambiar 10 por numero real
	motor[gripperTop] = 0;
}
...
//Proceso de brazos
task setArm{ // Andres Comment - Sets arm based on only one side -> armX considera ambos brazos
...
//sistema de stacking
task stackCone{
	//Sube la torre mientras hay conos alfrente
	while(SensorValue(towerSensor) < 10){ // reemplazar 10 por distancia en cm de la torre de conos
		motor[DRLU] = drSpeed;
    motor[DRRU] = drSpeed;
	}
	motor[DRLU] = 0;
	...
	//mueve el cono hacia atras
	armSpeed = 127; 
	armTarget = 0; // reemplazar por target real
	startTask(setArm);
	
	if(armStat){
		//suelta el cono
		roll(false);	
		//los brazos regresan alfrente
		armTarget = 0; // reemplazar por target real		
		startTask(setArm); 
	}
	//baja el double reverse
	drTarget = drMin;
	drSpeed = 127;
	setDR();
}
...
task moveBase
	{
		...
		while(SensorValue(baseL) < baseTarget){ // Andres Comment - Target based only on left -> se corrige usando encDif
...
task usercontrol()
{
  ...
  //UserControlCodePlaceholderForTesting();
	...
	// Anadir boton para sistema de stacking
